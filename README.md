...
- 웹 사이트 앞쪽에 배치하여 사이트 및 어플리케이션 등을 노린 공격을 방어하는 보안대책

## 대역폭

- 네트워크 연결의 폭
- 대역폭이 높을수록 많은 양의 데이터를 전송할 수 있다


## AWS
 - VPC : 가상네트워크 환경, 해당 환경내에서 여러 서브넷을 구축하여 pulbic 서브넷, private 서브넷을 구축할 수 있다.
 - 보안 그룹 : 각 서브넷에 접근 가능한 포트를 지정하는 것. 서버의 방화벽 보통 private Subnet은 외부 접근을 금지하고 public subnet에 필요한 포트만 열어 보안적으로 유리함을 가져감
## ECR 
 - 도커 허브처럼 컨테이너 이미지를 관리하는 주체
  - AWS 환경과 연동하여 네트워크 트래픽 감소 및 도커허브보다 빠른 속도로 배포를 진행할수 있음
 - 기존 배포방식은 해당 서버에 런타임 환경 구축이 필요하지만 컨테이너 이미지를 이용하면 런타임 환경까지 동일히게 구축할수있음
  - 언제나 동일 런타임 환경 보장이라는게 생각보다 큼
## ECS
 - 컨테이너 오케스트레이션 서비스로 컨테이너화 된 어플리케이션을 배포할수 있는 서비스
 - 해당 기능으로 컨테이너 라이프사이클을 관리 할 수 있음
## TCP 
 - Keep alive : 주기적으로 작은 단위의 패킷을 전송하여 헬스체크를 진행합니다 
   여러 요청을 보낼때 핸드쉐이크 등의 단계를 넘어갈수 있음
 - 소켓통신 또한 TCP 통신이며 dbms와 어플리케이션 간의 커넥션 풀도 소켓통신임
 - Dynamo DB등 특정 db는 http 통신이나, 위의 기능을 통해 연결성을 유지하는듯? (확인해봐야함)

## 부하분산 
웹 사이트의 성능을 높이고자 여러 대로 웹 사이트를 분산처리하는 방식 
### DNS 라운드 로빈

<img width="792" height="372" alt="image" src="https://github.com/user-attachments/assets/5e0d9243-329d-4740-948c-3f801ed900c8" />

로드밸런서를 사용하지 않고, DNS 설정만 가지고 호스트 하나에 IP 주소를 여러개 설정 하여 부하를 분산하는 방법
 - 해당 서버의 부하 상태와 상관없이 균등하게 할당되므로 각 컴퓨팅 서버의 처리능력이 고려되지 않음
 - 다운된 서버의 IP를 반환 할수도 있음

### NAT 형
VIP 라는 가상 IP 주소에 대한 요청을 실제 서버 여러 대에 할당하는 방식, 일반적인 로드밸런서(L4)에서 채용 됨


### GSLB (Global Server Load Baancing)


<img width="650" height="445" alt="image" src="https://github.com/user-attachments/assets/f53927b6-88fb-4d9b-84fa-caac48f18e64" />

DNS 라운드 로빈 방식에서 문제 되었던 장애 발생한 서버에도 할당될수 있는 문제점을 해결한 DNS 방식
 - 각 컴퓨팅 서버에 대한 상태체크를 진행 해 다운된 서버를 알 수 있음 (모니터링)
 - 각 사이트 주소에 대해 지리적 (IP주소를 통해)으로 가까운 서버로 IP 응답
 - 서비스 요청에 대한 응답속도가 얼마나 빠른지 레이턴시 값을 토대로 분산처리 ( 해당 방식은 고성능 서버에 요청이 몰리므로 DNS 라운드 로빈에서 발생한 분산문제도 해결 할 수 있음)
